...
const KNOWN_STRATEGIES: Strategy[] = [
  {
    id: 'mrz_pace',
...
const UNKNOWN_STRATEGIES: Strategy[] = [
  {
    id: 'can_mrz',
    label: 'CAN + MRZ',
    desc: 'PACE avec CAN et données MRZ complètes',
    type: 'I',
    needs: ['can', 'mrz'],
  ...
  },
  {
    id: 'skip_pace_mrz',
    label: 'Sans PACE + MRZ',
    desc: 'Contourne PACE, lit directement avec MRZ — utilisé par nfc-scan-modal',
    type: 'P',
    needs: ['mrz'],
    buildParams: (_can, docNumber, dob, expiry) => ({
      documentNumber: docNumber,
  ...
  },
  {
    ...
    desc: "PACE avec données vides — la carte accepte-t-elle n'importe quoi?",
    type: 'I',
    needs: [],
    buildParams: () => ({}),
  },
  {
    id: 'open',
    label: 'Sans PACE ni auth',
    desc: 'Aucune authentification — carte entièrement ouverte?',
    type: 'P',
    needs: [],
    buildParams: () => ({}),
  },
...
const ALL_STRATEGIES = [...KNOWN_STRATEGIES, ...UNKNOWN_STRATEGIES];
...
export function NfcDiagnosticCard() {
  ...
  return (
    <View style={styles.card}>
      ...
      {/* ── Carte confirmée (type 2) ── */}
      <View style={styles.groupHeader}>
        <Text style={styles.groupHeaderText}>Carte confirmée</Text>
        <Text style={styles.groupHeaderHint}>Remplir CAN + les 3 champs MRZ</Text>
      </View>
      {KNOWN_STRATEGIES.map(renderStrategy)}

      {/* ── Carte inconnue (type 1) ── */}
      <View style={[styles.groupHeader, { marginTop: 8 }]}>
        <Text style={styles.groupHeaderText}>Carte inconnue</Text>
        <Text style={styles.groupHeaderHint}>Tester toutes les méthodes d'accès</Text>
      </View>
      {UNKNOWN_STRATEGIES.map(renderStrategy)}
    </View>
  );
}
